...
- `charts/n8n/configurations/production/values.yaml` (alongside the base `values.yaml`) holds the Bitovi Platform deployment's actual settings: queue mode, bundled Valkey, community nodes and metrics on, `OnePasswordItem`-backed secrets, and the Ingress for `n8n.bitovi-tools.com`.
- **Postgres is deliberately not a template in this chart.** The in-cluster CNPG `Cluster` is composed in from `bitovi-platform-services`' shared `charts/platform-app-resources` chart, as a second ArgoCD Application source (see that repo's `gitops/apps/n8n.yaml` and `pto-api.yaml` for the established pattern) — not duplicated here per-app. `OnePasswordItem` secrets do stay in this chart, matching the precedent `pto-api` already set for app-owned secrets.
- **DNS/TLS aren't composed from anywhere at all.** Per [ADR-0002](https://github.com/bitovi/bitovi-platform-services/blob/main/docs/adr/0002-wildcard-cert-external-dns.md) in `bitovi-platform-services`, this is a zero-per-app-resource platform feature: a wildcard ACM cert for `*.bitovi-tools.com` and `external-dns` watching every Ingress already exist platform-wide. Declaring `host: n8n.bitovi-tools.com` on the Ingress (see `configurations/production/values.yaml`) with 443 in `listen-ports`, and no `certificate-arn` annotation, is the entire app-side contract.
